...
public class TIGDataBase {
	...
	public static void createTablesDB() {
		try {   				
			...
			query = "CREATE TABLE Image (id INTEGER PRIMARY KEY AUTOINCREMENT," + 
				"name char(50) NOT NULL," +
				"path char(80) NOT NULL," +
				"noaccents char(50) NOT NULL)";
			stmt.addBatch(query);

			query = "CREATE TABLE Asociated (concept INTEGER NOT NULL " +
				"CONSTRAINT fk_concept REFERENCES Concept(id)," + 
				"image INTEGER NOT NULL " +
				"CONSTRAINT fk_image REFERENCES Image(id)," +
				"PRIMARY KEY (concept,image))";
			stmt.addBatch(query);
			stmt.executeBatch();
	...
	public static int updateConceptDB(String oldConcept, String newConcept) {		
		...
		try {
			...
			if (!rsIm.next())
			{
				...
				if (!rsIm.next()){
					insertConceptQuery = "INSERT INTO Concept (word,noaccents) VALUES (\"" + newConcept + "\", \"" + order(newConcept) +"\")";
					stmt.executeUpdate(insertConceptQuery);
				}
			}
			else
			{
				rsIm = stmt.executeQuery(newConceptQuery);
				if (!rsIm.next()){
					updateConceptQuery = "UPDATE Concept SET word = \"" + newConcept + "\", noaccents = \"" + order(newConcept) + "\" WHERE word = \"" + oldConcept + "\"";
	...
	/**
	 * Delete all the associations of the image with the id indicate by <code>image</code>.
	 * 
	...
	public static void insertImageDB(String name, String path) {		
		...
		try {
			...
			if (!rsIm.next())			{
				...
				insertImageQuery = "INSERT INTO Image (name,path,noaccents) VALUES (\"" + name +
								   "\",\"" + path + "\",\"" + order(name) + "\")";
	...
	public static void insertAsociatedDB(int concept, int image) {		
		...
		try {
			...
			//Hacemos lo mismo con el concepto
			asociatedQuery = "SELECT * FROM Asociated WHERE concept = \"" + concept + "\" " +
							 "AND image = \"" + image + "\"";
			ResultSet rsIm = stmt.executeQuery(asociatedQuery);
			if (!rsIm.next())
			{
				insertAsociatedQuery = "INSERT INTO Asociated (concept,image) VALUES (\"" + concept + 
									   "\", \"" + image + "\")";
	...
	public static Vector imageSearch(String imagen){
		...
		try{
			...
			Statement stmt = conn.createStatement();
			String query = "SELECT * FROM Image WHERE name LIKE \"" + imagen.replace('*', '%')  + "\" " +
	...
	/**
	 * Delete the association between the concept with the id indicate by <code>concept</code>
	 * and the image with the id indicate by <code>image</code>.
	 * 
	 * @param concept the concept
	 * @param image the image
	 * 
	...
	public static boolean areAsociated(int concept,int image){
		...
		try{
			//Creamos un objeto Statement que se conectara a la BD
			Statement stmt = conn.createStatement();
			query = "SELECT * FROM Asociated A " +
				    "WHERE A.concept = \"" + concept + "\" AND A.image = \"" + image + "\"";
			res = stmt.executeQuery(query);
	 ...
	 * Search.
	 * 
	 * @param keyWord1 the key word1
	 * @param searchOptions1 the search options1
	 * @param keyWord2 the key word2
	 * @param searchOptions2 the search options2
	 * @param keyWord3 the key word3
	 * 
	 * @return the vector
	 */
