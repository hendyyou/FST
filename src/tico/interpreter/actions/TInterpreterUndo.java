...
public class TInterpreterUndo extends TInterpreterAbstractAction{

	
	public TInterpreterUndo (TInterpreter interpreter){
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (TInterpreter.AccumulatedList.size()==0){
			System.out.println("IMPOSIBLE DESHACER!");
		}
		else 
		{	
			interpreter.accumulatedArea.remove(TInterpreter.AccumulatedList.size()-1);
			TInterpreter.AccumulatedList.remove(TInterpreter.AccumulatedList.size()-1);
			interpreter.accumulatedArea.update(interpreter.accumulatedArea.getGraphics());
		}
		TInterpreter.TStart =new TThreads(new TInterpreterRun(interpreter));
