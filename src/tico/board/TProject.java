...
public class TProject {
	...
	/**
	 * Creates a new empty <code>TProject</code> with the specified initial
	 * <code>name</code>.
	 * 
	...
	/**
	 * Returns the project <code>name</code>.
	 * 
	...
	/**
	 * Sets the project <code>name</code>.
	 * 
	...
	/**
	 * Creates a new project <code>name</code> different from any other project
	...
	/**
	 * Add a <code>board</code> to the project. The board is renamed if its
	...
	/**
	 * Removes the specified <code>board</code> from the project.
	 * 
	...
	/**
	 * Returns the <code>initialBoard</code>.
	 * 
	...
	/**
	 * Sets the <code>initialBoard</code>. It <code>initialBoard</code> is not
	...
	/**
	 * Determines if any project board has the specified <code>name</code>.
	 * 
	...
	/**
	 * Determines if any project board different from <code>board</code>
	 * has the specified <code>name</code>.
	 * 
	...
	/**
	 * Adds an <code>ProjectChangeListener</code>.
	 * 
	 * The <code>ProjectChangeListener</code>. will receive a
	 * <code>ProjectChangeEvent</code> when the project of any of its boards
	 * has been changed.
	 * 
	 * @param listener The <code>ProjectChangeListener</code> that is to be notified
	 */	
	...
	/**
	 * Removes a <code>ProjectChangeListener</code>.
	 * 
	...
	/**
	 * Generates an XML <code>Element</code> that contains the project information.
	 * 
		...
		if (element.getTagName().equals("project")) {
			...
			for (int i = 0; i < childNodes.getLength(); i++) {
				...
				if (tagName.equals("board"))
					project.addBoard(TBoard.XMLDecode((Element)childNodes
							.item(i)));
				else if (tagName.equals("initial_board"))
					initialBoardName = childNodes.item(i).getChildNodes()
							.item(0).getNodeValue();
			}
			...
			for (int i = 0; i < project.getBoardCount(); i++) {
				TBoardModel boardModel = (TBoardModel)project.getBoard(i)
						.getModel();
				for (int j = 0; j < boardModel.getRootCount(); j++) {
					...
					for (int k = 0; k < component.getChildCount(); k++) {
						TComponent childComponent = (TComponent)component
								.getChildAt(k);
						if (childComponent.getAttributes()
								.containsKey(TBoardConstants.FOLLOWING_BOARD))
			...
			for (int i = 0; i < followingBoardComponents.size(); i++) {
				...
				TBoard followingBoard = project.getBoard((String)component
